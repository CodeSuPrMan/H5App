...
//          },false);
		...
		function checkForm() {
			...
			if(window.localStorage['moduletype']==1){
				if(!$scope.checkdetail.CheckContent || ($scope.checkdetail.CheckContent && ($scope.checkdetail.CheckContent == null || $scope.checkdetail.CheckContent == ""))) {
					$ionicLoading.show({
						template: '检查情况/问题隐患不能为空！',
						duration: 1000
					});
					return false;
				}
				if(!$scope.checkdetail.SuperviseSuggest || ($scope.checkdetail.SuperviseSuggest && ($scope.checkdetail.SuperviseSuggest == null || $scope.checkdetail.SuperviseSuggest == ""))) {
					$ionicLoading.show({
						template: '监督要求/建议不能为空！',
						duration: 1000
					});
					return false;
				}
